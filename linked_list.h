...
/*
 * create the list
 * insert elements (back-front, at any position)
 * get the elements
 * find the element
 * remove the value (without front and back)/remove at any position
 * size/is_empty/destroy
 *
 * create the iterator
 * check if it has a next value
 * function that moves the iterator forward (iterator_next)
